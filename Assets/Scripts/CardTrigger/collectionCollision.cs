...
public class collectionCollision : MonoBehaviour
{
    ...
    public void SetSelfToSock()
    {
        ...
        // 确保插槽列表已初始化
        if (toolBoxManager.collectionSocketInteractorList == null || toolBoxManager.collectionSocketInteractorList.Count == 0)
        {
            Debug.LogWarning("collection插槽列表为空，正在搜索所有插槽...");
            toolBoxManager.SearchAllSocketInteractors();
        }

        // 使用ToolBoxManager的强制插入方法，将当前游戏对象插入到可用collection插槽中
        bool success = toolBoxManager.ForceInsertToAvailableCollectionSocket(this.gameObject);

        if (success)
        {
            Debug.Log($"成功将 {this.gameObject.name} 插入到collection插槽中");
        }
        else
        {
